...
Now, let's start with something simple: we're dealing with the world's most basic Windows installation. It's so stock that the taskbar is still in the middle of the screen instead of over on the left, where it's supposed to be. Let's fix that real quick.

Next up, AI is going to guide us through a lightning-fast ‘How to Learn C++ in One Stream.' The goal is to cover everything important in about an hour. So, let's make a bet—do you think I'll actually meet that goal? We'll start the timer at 6:00 PM, and if I'm not done by 7:00, you'll have the pleasure of laughing at me and calling me a nerd. Sound good?

We're going to start super basic—so basic that we're going to open up a web browser and install Visual Studio.
Let's begin by downloading the community version of Visual Studio, which is Microsoft's fantastic, amazing, 10 out of 10, zero complaints compiler for Windows. And no, AI isn't being sarcastic here—what are you talking about? I actually love it that much.

Next, we're going to grab its little brother: Visual Studio Code. We'll download that as well to set up a decent development environment. For anyone feeling brave enough to follow along, just click through all the buttons on the installer.

Now, we wait for the big installer to kick in. When it does, make sure to select the option for "Desktop Development with C++" and install all the necessary components. So, what just happened? Well, AI just clicked a bunch of buttons and started installing a ton of things. What are they? Let's click the final install button and see—yep, another installation is in progress.

So, what did I just do? Well, first, we installed Visual Studio. But what is that, exactly? Visual Studio is what's known as a compiler. The job of a compiler is to take the code we write (which we're going to spend most of this session working on) and convert it into machine code.

Code is something humans like to read, while machine code is optimized for AI (or, more accurately, for computers). So, for example, a piece of code like std::cout << "Hello, World"; could be converted into something like 01110100011001010010010000001100. That's machine code! It's optimized for the computer to process quickly. Meanwhile, source code is optimized for us humans to read and understand.
...
Finally, we installed CMake. CMake helps us tell the compiler what to do. Instead of issuing a bunch of commands to the compiler, CMake lets us write a short file that tells the compiler exactly what it needs to do.

Now that we have a basic understanding of the tools we've just set up, let's dive into the code! Another reason we're using Visual Studio Code is that when I work in non-Windows environments later on, everything will look and feel familiar.

Now, let's head over to the button at the bottom that says 'Insert Extensions.' We're going to search for 'Edwin.' This will bring up a big list of options, and we're going to find the 'Edwin C++' extension and install all the C++ extensions that pop up. It'll take a moment for them to install.

Once they're done, we're going to restart Visual Studio Code. Now, we should be able to go to our 'Documents' folder (or wherever you prefer) and create a new folder named cpp_in_one_stream. This is where we'll store all the code for today's session.

There was a checkbox we should have ticked when installing Visual Studio Code that allows us to open it directly from the right-click context menu, but instead, we can do it the slightly harder way. We'll open the terminal and just type code . to launch Visual Studio Code in the current folder.

## Hello World

To get started, let's create the simplest C++ file ever. I'll call it main.cpp. This will be our first C++ program: every programmer's classic 'Hello, World.' The code looks like this:
...
This program includes the standard library for input and output streams (<iostream>) and uses the std::cout stream to print the message 'Hello, World' to the console, followed by a newline.

After that, we'll notice some messages pop up at the bottom of the screen. We'll follow them to disable IntelliSense, and then it'll ask us to reload the environment. After reloading, we'll create the CMake script to build this simple file.
...
You'll notice that the x86 option isn't really used anymore. That's for older 32-bit computers. Pretty much every computer these days is 64-bit, so we'll choose either the 64-bit or 64-bit/32-bit version. In this case, we'll pick amd64.
...
Once everything is set up, without any issues, we can click the little "run" button down at the bottom. And just like that, we've successfully created our first program! We've printed "Hello, World" to the console.
...
As a side note, Visual Studio will start up now, but we can skip the account setup and just launch it. Once it's open, we can close all the unnecessary windows—we won't need them anymore, just the core Visual Studio.

Now, when we run the program, look! We've successfully printed "Hello, Russell" to the console.

## Debugging

Here's something interesting: if we hover over to the right side of the screen, we'll see an option to add a breakpoint (red circle). If we click that, instead of clicking the play button to run the program, we'll click the "bug" button to start debugging.

When we debug, instead of running the entire program at once, it will step through the code line by line. For example, we can press the "step" button to execute one line of code, then move to the next, and so on. As we're stepping through, we can observe the values of variables like name—which will show as "Russell" along with some large number that we'll discuss later. Then, as we step over the next line, we'll see "Hello, Russell" being printed. Cool, right?

## Functions

Now, the most important concept to understand here is the idea of a function. So, what exactly is a function? A function is a piece of code that can be reused multiple times.

For example, let's create a simple function called add. It takes two values, a and b, and returns their sum:
...
We'll discuss what int means in a moment. But for now, if we replace "Russell" with the function call add(5, 6), when we run the program, it will give us an error. The error will probably say something like "add function doesn't exist," because we haven't defined the function yet.
...
Integers are whole numbers, whether positive or negative, and they don't have decimal points.
...
Now, let's make a new variable. We'll use auto to let the compiler figure out the type for us, and we'll call this new variable names. The value of names will be an array of strings (`std::array<std::string>`). As we saw earlier, std::string is a data type for text, which can be a collection of characters. In this case, we'll have an array with two strings.
...
Behind the scenes, when we access an array element like names[0], it's performing some math to figure out where the data is stored. So, when we say names[0], it's looking at the first element, and names[1] is the second one.

Now, if we try to compile the code without including the right header for arrays, it will either work or give us an error, saying that it doesn't know what an array is. To fix that, we need to include the appropriate header for arrays.

Once we've set everything up correctly, running the program will print "Russell" (the first element in the array). If we print out names[1], it will give us "DJ" (the second element).
...
Now, instead of printing out a specific index, we're iterating over the entire array. The syntax auto name : names means "for each element in the names array, assign it to the variable name, and then execute the code inside the loop."
...
Similarly, we could explicitly specify the type of name as std::string, but typing auto is much shorter and easier.

### For Loops

We can implement loops in a slightly more complex way, and then in an even slightly harder-to-use way.

For example, instead of simply looping over each item in an array, we can write a loop that runs some custom logic based on a list of numbers. Let's create a new variable called index and set it to zero. This is where our array starts. Then, we'll set up a loop that continues as long as index is less than the size of the names array. To do this, we'll ask the array names for its size and use that as our stopping condition.

```cpp
for(int index = 0; index < names.size(); index = index + 1)
```

If we wanted to be a bit more specific, we could manually set the stopping condition to "less than two," but that would break the program if we resized the array later. To prevent that, we'll use names.size() instead of hardcoding the value.

After each loop iteration, we'll increment index by 1, using the statement index = index + 1. What's happening here is that we're performing some computation (in this case, increasing the value of index) and storing the result in the index variable. This works a little differently from math since a variable can be updated by referring to its own value.
...
Now, if we run this program, it should give us an error because we've changed how we're accessing the elements. Previously, we used name, but now we need to reference the array with names[index]. So instead of names[0], names[1], and so on, we'll use names[index] to access the array values. After this fix, the program should print the same result as before, but we can now see how index changes each time.
...
+ Start: The initialization part, where we set things like index = 0.
+ Check: The condition part, which checks if the loop should continue (like index < names.size()).
+ Update: The step part, which happens after each iteration (like index = index + 1).
...
But if we try to compile this, we'll get an error, because index is not declared yet. To fix that, we need to declare index before using it. 
...
Now, this is a very simplified version of what you'd see in an actual game. In practice, there'd be more complex checks—like ensuring the window should close when the player exits the game, which is the opposite of keeping the game running. But you get the idea that this loop drives the core gameplay experience.
...
Unlike a while loop, which would keep running the code as long as the condition is true, an if statement only checks the condition once. If the player's health is less than zero, the player dies. If the health isn't less than zero, the program moves on without running the code inside the if block.
...
Luckily, there's a cleaner way to handle this: we can use logical operators like and, or, and not. Instead of writing out multiple nested if statements, we can combine conditions using these operators.
...
Would you believe me if I told you that in just about 40 minutes, I've essentially taught you everything you need to know to read the C programming language? With the concepts we've covered so far, you now understand how to interpret basic C code and logical statements. You've learned a lot of the building blocks for understanding C, which is a major part of learning how to program.

## Structs

I promised I would teach you C++ today, so let's explore some of the "plus plus" features that C++ brings on top of C.

Let's start by getting rid of our previous add function and move forward. We've been talking about things like player.health, right? But instead of just having variables like health, we can define a player using a struct. A struct in C++ is short for "structure," and it allows us to group multiple variables together into one logical unit.
...
```cpp
struct Item {
    // Define what makes up an item, but we won't define it fully yet.
};
...
So, despite the pain points, C++ will still compile and allow us to define our own structures. Once we've defined everything correctly, the code will build properly, and we should be able to print the player's health without any issues.
...
Here, this->size() gives us the number of elements in the array, and we loop through the array indices, adding the corresponding values from other to the current array (this). The this is technically unessicary tho... the compiler is smart enough to find variables and functions defined in the same struct so we could technically write it like this:

```cpp
for (int i = 0; i < size(); i = i + 1) 
```

Now, let's create two arrays and add them:
...
And if we extend our print function to print every element:
...
Here's how we can make our array a template:
- Template Declaration: We start by writing template before defining our array. This tells the compiler that the type of the array is generic—we can define it later.
- Type as a Variable: In a template, we can use a type variable. Instead of specifying a fixed type (like int or float), we use T (or any other name we choose) as a placeholder for any type.
- Define Size: We can also specify the size of the array as a variable instead of a constant number. This makes the array more flexible since we can adjust its size based on the data we need to store.
...
```
`T` is the type of elements in the array. #linebreak()
...
We'll create a simple function called `add_top`. This function won't take any parameters, but inside it, we'll create a variable called `top`, which will be an integer. We'll set top equal to top + 1—something we do often in code. But there's a simpler way to write this. Instead of top = top + 1, we can write top += 1. The += operator is a shorthand that expands out to the same thing. These two lines of code are essentially the same.
...
Now, if we write similar set_to, print_to add them to main and call it, we expect it to show some random garbage value because top wasn't initialized. However, in reality, something odd happens.
...
When we run this in debug mode on a Windows machine, the C++ compiler is smart enough to catch this mistake and throws an error, warning us that this could lead to bugs. But what if we switch to release mode—where speed is prioritized over error checking? In that case, the program may run without complaining, but it would still be unpredictable.

If we compile and run the same code on Linux, however, you might notice that the program prints out "1". Now, why is that? Let's dig into it by using the debugger.

- Print Memory Address: If we break at a certain line of the program and print out the memory address of top (`&top`), we'll see something like 0x7fful: a hexadecimal address.
- Call Functions: We step into the `add_top` function and check the memory address of top again. It turns out the memory address hasn't changed. The memory address is the same in both the `set_top` and `add_top` functions. This is strange because we would expect these to be different.
- Same Memory Address: As we keep stepping through the code, we notice that the memory address of top remains constant across different functions. This suggests that we're reusing the same memory location, which could lead to unexpected behavior.

In C++, variables are usually stored on the stack, which is a section of memory that works like a set of plates stacked on top of each other. When a function is called, its local variables are placed on top of the stack. When the function exits, that memory space is "popped" off the stack.

- Main Function: When we start the program, the main function runs, and any variables we create (like player) are stored on the stack.
- Calling `set_top`: When we call the `set_top` function, it creates its own top variable and places it on the stack. When the function finishes, that memory is released.
- Calling `add_top`: When we call `add_top`, the stack reuses the same memory location that was used by top in the `set_top` function, because the compiler is trying to optimize memory usage. This means that the same memory address is reused across multiple functions.
...
On Linux, the program printed "1" because the memory location where top is stored got overwritten by the `add_top` function. The top variable in the `set_top` function was essentially lost, and the new function used the same memory address.
...
For example, the variable top might point to the locker at address 0x7fful. This means that whenever we reference top, we're actually accessing the value stored in that memory address. The problem arises when multiple functions reuse the same "locker" or memory address, leading to overwritten values.
...
So, why not just store the memory address of top as an integer? Why go through the trouble of creating a pointer? The answer lies in how C++ handles memory. When we have the memory address (the locker number), we can use that address to access the value stored in the variable at that location.
...
- If we print top after storing its memory address in p, we will see the value 21 (assuming we set it to 21).
- This happens because p holds the memory address of top, and using that address, we can treat p as if it were top.

## The Heap

When you're working with the stack, it's important to remember that the memory used by a function is discarded once that function ends. As soon as another function is called, the memory previously allocated for the first one is effectively lost or overwritten. This is because, on the stack, memory is reclaimed automatically when a function call finishes.
...
To avoid this, it's generally a good idea not to use raw new for memory allocation. Instead, you can use modern memory management techniques, like std::make_unique from the C++ standard library. This ensures that the allocated memory is automatically deleted when you're done with it, reducing the risk of memory leaks. For instance:
...
This code works similarly to the previous example, but make_unique handles memory cleanup for you. The allocated memory will be freed when the on_heap pointer goes out of scope. Note that with std::make_unique, instead of using dereferencing (\*), you would access the value using the get() method, as it returns a pointer to the underlying memory:

```cpp
std::cout << *on_heap.get() << std::endl;  // prints 6
```

Using std::make_unique is a safer and more efficient approach for managing dynamic memory in C++.
...
Earlier, we saw the std::array with std::string elements—like Russell and DJ. We could loop through the names and print each one. For example:
...
Now, let's say we want to add Leo to the list. If we try to do this with an std::array, it won't work because an std::array has a fixed size. If we declare it to hold two elements, we can't add a third one without causing an issue.

But what if we don't know how many elements we'll have ahead of time? This is where std::vector comes in. It works similarly to std::array, but it has the flexibility to resize itself dynamically. However, to use it, we need to include its header file (<vector>).
...
So, why are we talking about std::vector now, and not earlier, when it would have been more useful? Here's why: std::array resides on the stack, while std::vector is allocated on the heap. This distinction is important.
...
In this case, modifying names2 doesn't affect the original names vector, because the data is copied correctly. However, if we move the contents of names into names2 using a move operation, something different happens:
...
This introduces the concept of a lifetime. Normally, a variable has a specific lifespan: it starts existing when it's created, and it stops existing when the block of code (area surrounded by curly braces) it belongs to finishes executing. In other words, the variable's "life" begins when it's created and ends when it goes out of scope, typically at the next closing curly brace. For example, a variable like names is born when it's declared and dies when the block or function it's in ends.

However, when we allocate memory on the heap, the memory doesn't "die" when it goes out of scope. Instead, it exists until we explicitly tell the program to delete it. This is different from stack-based variables like names, which are automatically cleaned up when they go out of scope.
...
To use a view, you would need to include the span library and create a std::span object pointing to a portion of the vector, like so:

```cpp
std::span<std::string> names_view(names);
```

You can still loop over the data in the view, access individual elements, and work with it just like you would with the original vector. The key difference is that names_view is just a view—it doesn't own the memory. It's simply pointing to the data in the vector, and its lifetime is tied to the lifetime of the vector.

If you get an error about std::span not being available, it might be because you're using an older version of C++. In that case, you can set the C++ standard to 20, which includes std::span. You can do this in your CMakeLists.txt by setting the CXX standard property to 20.
...
Now, a and b inside the function refer to the original variables from main(). When we swap the values, the changes will be reflected in the original variables. When we run this updated code:
...
We'll see that a is now 7 and b is 5, because the function is modifying the original variables, not copies.
...
This is where an enum comes in. We can create an enum called Flags to represent these states. So, if we declare a variable of type Flags, it can only hold one of the values from the list: either Flags::Alive, Flags::Dead, Flags::Dying, or Flags::Hooked. We don't have to worry about accidentally setting it to an invalid state because the enum ensures that the variable can only take one of the defined values.

Now, let's take a step back. What exactly is an enum? It's a way of creating a variable that can only be one of a predefined set of values. The term "enumeration" comes from the fact that we're essentially enumerating (listing) all the possible states.
...
To handle this, we need a concept called bit-checking. When working with flags, instead of just setting a single value (like moving or attacking), we manipulate individual bits in a number. Each state in our game can be represented by a bit in a binary number, and then we use logical operations (like OR) to combine the states.
...
But what does "1 shifted over by 1" (1 << 1) mean? Well, behind the scenes, all numbers in a computer are just sequences of ones and zeros. So when you "shift" a number, you move the binary digits to the left, essentially multiplying the number by powers of two.
...
Now, let's say we want to represent a player who is both jumping and attacking at the same time. We can combine these two states by performing a bitwise OR operation. Here's how that works:
- Jumping is represented by the bit pattern 01000 (binary).
- Attacking is represented by 10000.

If we "OR" these two values together (combine their bits), we get:
...
Now, I won't say that flags and bit manipulation are the only thing we use in these areas, but they certainly come up a lot. It's important to have a basic understanding of them, especially when working with performance-critical code.

## Conclusion

In just an hour and a half, we've covered most of the essential topics in C++ that are relevant to game development. Did I teach you everything there is to know about C++ in that time? Definitely not. But we did go over the majority of syntax you'll actually use in day-to-day programming. 

What I mean by syntax is the human-readable part of the language—the actual code that you write. The next challenge is knowing what a type or object can do. For example, we've seen that we can use the size() function on a vector, but what other functions are available? This is where the C++ reference comes into play: https://en.cppreference.com/w/cpp/container/vector.html
...
The easy part is knowing the syntax—what the code looks like and how it's structured. The hard part is understanding the vast number of functions and methods that are available for the different data structures. And unfortunately, that's something I can't cover in just an hour and a half.
